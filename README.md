...
از پنل ادمین: **پیام‌رسانی → تنظیمات توییتر (X)**

- کلیدهای API، فعال/خاموش، ارسال سیگنال/نتیجه، VIP، و متن CTA
- دکمه **ارسال توییت تست** برای بررسی اتصال
- به‌صورت پیش‌فرض فقط سیگنال‌های عمومی (`all`) پست می‌شوند

کلیدهای `.env` فقط به‌عنوان پیش‌فرض اولین ایجاد رکورد استفاده می‌شوند؛ مدیریت اصلی از پنل است.
